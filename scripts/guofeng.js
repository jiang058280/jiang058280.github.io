...
function displayTitle(title) { return String(title).replace(/^(?:[A-Z]\d{3}|\d{1,3})(?:[.．、_\s-]+|(?=[\u4e00-\u9fffA-Za-z]))/,''); }
function card(post) { return `<a class="gf-article" href="/${escape(post.path)}"><span>${escape(post.categories.map(c => c.name).join(' · ') || '技术手记')}</span><h3>${escape(displayTitle(post.title))}</h3><span class="gf-arrow" aria-hidden="true">↗</span></a>`; }
